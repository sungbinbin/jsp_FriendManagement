...
public class SaveFriendServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		response.sendRedirect("./friend/addFriend.jsp"); // get 방식으로 호출을 할 경우에는 저장할 친구 데이터가 업기 때믄에 친구데이터를 입력 받는 페이지로 강제
															// 이동
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("friendName") != null || !request.getParameter("friendName").equals("")) {
			name = request.getParameter("friendName");
		}
		...
		try {
			...
			} else if (e instanceof SQLException && msg.contains("NULL")) {
				System.out.println("이름이 null!!!!!!!!!!!!!!!!!!");
				redirectPage = request.getContextPath() + "/friend/addFriend.jsp?status=nameNull";
			}
